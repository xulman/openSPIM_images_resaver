...
def fileName(Folder, c,p,t,z):
	return (inDirStr+Folder+os.path.sep+os.path.sep+filePattern).format(c,p,t,z)
...
def processFolder(Folder):
	...
	print("Positions: 0 - "+str(maxPosition)+" (including missing files)")
	print("     Time: 0 - "+str(maxTime)+" (including missing files)")
	print(" Z slices: 0 - "+str(lastAvailZ))

	for t in range(maxTime+1):
		for c in range(lastAvailChnnl+1):
			for p in range(maxPosition+1):
				# test if this combination of time,channel,position is valid...
				if not os.path.exists(fileName(Folder, c,p,t,0)):
					continue
				#
				...
				logger.write(fileOutputPattern.format(timeShift,c,p)+" = "+(inDirStr+Folder+os.path.sep+os.path.sep+reportPattern).format(c,p,t)+"\n")
				logger.flush()
